...
public class DeleteStudent extends DynamicMenuScreen{
    ...
    public DeleteStudent(Datasource datasource, StudentDao studentDao) {
    ...
    private DynamicNavigationHandler createDynamicHabdler(Datasource datasource, StudentDao studentDao) {
        return screenName ->{
            ...
            if (parts.length != 2 || !"deleteStudent".equals(parts[0])) {
                throw new IllegalArgumentException("Lens is" + parts.length);
            }
