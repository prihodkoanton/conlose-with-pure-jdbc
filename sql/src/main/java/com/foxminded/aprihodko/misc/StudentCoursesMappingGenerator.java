...
public class StudentCoursesMappingGenerator {
    ...
    public void mapStudentsToCourses(Connection connection, int minCourseCount, int maxCourseCount) throws SQLException{
        Random rnd = new Random();
        List<Course> courses = courseDao.findAll(connection);
        if (courses.size() < minCourseCount) {
            throw new IllegalArgumentException("The number of courses is less than minCourseCount = " + minCourseCount);
        }
